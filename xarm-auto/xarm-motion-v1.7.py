...
class XArmController:
    ...
    def calculate_optimal_inspect_angle(self, object_angle):
        """
        Return object angle without modification for inspection.
        
        v1.7: Removed 90° rotation to test robot position alignment.

        Args:
            object_angle: Detected object angle in degrees (0-180)

        Returns:
            float: Object angle (no rotation applied)
        """
        # v1.7: Return object angle directly (no 90° rotation)
        return object_angle
    ...
    def inspect_sequence(self, target_det_x, target_det_y, object_angle=0.0, object_width=0.0, object_height=0.0, offset_x=CAMERA_OFFSET_X, offset_y=CAMERA_OFFSET_Y):
        ...
        try:
            ...
            camera_angle_adjustment = 0  # No adjustment needed
            ...
            if 0 <= angle_norm < 45:
                # Angle 0-44°: Approach with 0° offset
                default_direction_name = "Left"
                default_direction_offset = 0
            elif 45 <= angle_norm < 90:
                # Angle 45-89°: Approach with 90° offset
                default_direction_name = "Right"
                default_direction_offset = 90
            elif 90 <= angle_norm < 135:
                # Angle 90-134°: Approach with 0° offset
                default_direction_name = "Forward"
                default_direction_offset = 0
            else:  # 135-180°
                # Angle 135-180°: Approach with 180° offset
            ...
            # v1.7: Calculate robot yaw = object_angle + default_direction_offset
            # The offset was already determined based on angle range
            robot_yaw = object_angle + default_direction_offset
            
            print(f"[Inspect] ✓ SELECTED: Position ({gripper_det_x:.1f}, {gripper_det_y:.1f}) mm, Camera angle: {camera_angle:.1f}°")
            print(f"[Inspect] Robot yaw: {object_angle:.1f}° (object) + {default_direction_offset:.1f}° (angle-based offset) = {robot_yaw:.1f}°")
