...
class ClickDataManager:
    ...
    def write_click(self, x, y, button="left"):
        """Write a new click position with button type."""
        data = {
            "click_x": float(x),
            "click_y": float(y),
        ...
        print(f"[CLICK-{button_action.get(button, button).upper()}] Sent to robot: ({x:.1f}, {y:.1f}) mm")
            ...
            if mouse_clicked:
                ...
                for obj_data in object_data_list:
                    if point_in_polygon((mouse_x, mouse_y), obj_data['corners']):
                        ...
                        # Send object center position to robot with button type
                        click_manager.write_click(obj_data['x_mm'], obj_data['y_mm'], mouse_button)
                        break
